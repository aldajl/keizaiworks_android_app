...
public class NationalTab extends Fragment implements ActionBar.TabListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
			...
			loadTask = new LoadTask(getActivity(), url, tabContextRef, (TableLayout) getActivity().findViewById(R.id.national_stats_table));
		} catch (MalformedURLException e) {
			e.printStackTrace();
		}
        
        statsTable = (TableLayout) 
        		getActivity().findViewById(R.id.national_stats_table);		//Will be used to add/edit table
        db = new DatabaseHandler(getActivity());							//Gets the current Database to update
        userName_lbl = (TextView) 
        		getActivity().findViewById(R.id.national_username_lbl);
        user = db.getUser(1);
        ...
        final Button fetchDataBtn = (Button) 
        		getActivity().findViewById(R.id.national_fetch_data_btn);	//Fetch button to update db and post new data
